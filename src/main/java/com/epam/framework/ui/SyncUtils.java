...
public class SyncUtils {
    ...
    public static void waitUntilPageIsFullyLoaded(WebDriver driver, Duration timeOutInSeconds) {
        ...
        try {
            ...
            TestContext.getLogger().log(LogLevel.INFO,
                    String.format("Actual wait: %s milliseconds, Expected was %s",
                            (System.currentTimeMillis() - startTime), Duration.ofSeconds(10)));
        } catch (Exception ex) {
    ...
    public static void waitUntilClickable(WebDriver driver, WebElement element) {
        waitUntilClickable(driver, element, Duration.ofSeconds(WaitConstants.DEFAULT));
    }

    public static void waitUntilClickable(WebDriver driver, WebElement element, Duration waitInSeconds) {
        try {
    ...
    public static void waitForElementVisibility(WebDriver driver, WebElement element) {
        waitForElementVisibility(driver, element,Duration.ofSeconds(WaitConstants.DEFAULT));
    }

    public static void waitForElementVisibility(WebDriver driver, WebElement element, Duration waitInSeconds) {
        WebDriverWait wait = new WebDriverWait(driver, waitInSeconds);
        wait.ignoring(NoSuchElementException.class)
    ...
    public static void waitForElementInVisibility(WebDriver driver, WebElement element) {
        waitForElementInVisibility(driver, element, Duration.ofSeconds(WaitConstants.DEFAULT));
    }

    public static void waitForElementInVisibility(WebDriver driver, WebElement element, Duration waitInSeconds) {
        WebDriverWait wait = new WebDriverWait(driver, waitInSeconds);
        wait.until(ExpectedConditions.invisibilityOf(element));
    }

    public static void waitPresenceOfElement(WebDriver driver, By element) {
        waitPresenceOfElement(driver, element, Duration.ofSeconds(WaitConstants.DEFAULT));
    }

    public static void waitPresenceOfElement(WebDriver driver, By element, Duration waitInSeconds) {
        WebDriverWait wait = new WebDriverWait(driver, waitInSeconds);
